...
function ArrayFieldTitle({
  TitleField, idSchema, title, required,
}) {
  if (!title) {
    return null;
  }
...
class ArrayField extends Component {
  ...
  renderNormalArray() {
    ...
    const {
      ArrayFieldTemplate, definitions, fields, formContext,
    } = registry;
  ...
  renderFixedArray() {
    ...
    const itemSchemas = schema.items.map(
      (item, index) => retrieveSchema(item, definitions, formData[index]),
    );
